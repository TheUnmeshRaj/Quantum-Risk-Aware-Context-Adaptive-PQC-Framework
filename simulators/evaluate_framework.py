...
def _adaptive_choose(cap: float, threat_level: int, device_idx: int) -> int:
    ...
    - Matches security level to current threat
    - Respects hardware capability ceiling
    - Biases toward lower overhead at low threat (efficiency)
    - Escalates proactively for high-value devices
    """
    # High-value devices (hospital DB, PLC) get bumped up one tier proactively
    high_value_devices = {3, 4}  # Hospital DB, Industrial PLC
    urgency_boost = 1 if device_idx in high_value_devices else 0

    effective_threat = min(5, threat_level + urgency_boost)

    if effective_threat >= 4:
        if cap >= 8:  return 4
        if cap >= 5:  return 3
        if cap >= 2:  return 2
        return 1
    elif effective_threat >= 3:
        if cap >= 5:  return 3
        if cap >= 2:  return 2
        return 1
    elif effective_threat >= 1:
        if cap >= 5:  return 2
        if cap >= 2:  return 1
        return 1
    else:
        # Threat=0: only migrate if device is capable and high-value
        if device_idx in high_value_devices and cap >= 5:
            return 2
        if cap >= 8:  return 2
        return 0
...
def run_agent(env: PQCMigrationEnv, strategy: str):
    ...
    while not terminated:
        ...
        # info gives total counts but not per-device; we use average per-device costs
        n = max(num_devices, 1)
